...
const HotelCard = ({ room, index }) => {
  return (
    <Link to={'/rooms/' + room._id} onClick={() => window.scrollTo(0, 0)}  className='relative max-w-70 w-full rounded-xl overflow-hidden bg-white text-gray-500 shadow-[0px_4px_4px_rgba(0,0,0,0.05)]' >
      <img 
        src={room.images[0]} 
        alt="hotel" 
      ...
      />
      {index % 2 === 0 && (
        <p className='px-3 pt-1 absolute top-3 left-3 text-xs bg-white text-gray-800 font-medium rounded-full'>
          Best Seller
        </p>
      )}
